(() => {
  ...
  function isPostVslStep(item) { return item.event_name === 'step_viewed' && [2, 3, 4, 5, 6, 8, 9].includes(Number(item.step_number)); }
  function vslStartedIds(events) { return new Set(events.filter((item) => ['vsl_started', 'vsl_watch', 'vsl_progress', 'vsl_continue_unlocked', 'vsl_continue_clicked'].includes(item.event_name) || isPostVslStep(item)).map((item) => item.session_id)); }
  function vslContinuedIds(events) { return new Set(events.filter((item) => item.event_name === 'vsl_continue_clicked' || isPostVslStep(item)).map((item) => item.session_id)); }
  function countStage(events, step) { return step === 1.1 ? vslStartedIds(events).size : countFor(events, 'step_viewed', (item) => Number(item.step_number) === step); }
  ...
  function renderMetrics(sessions, events) {
    ...
    const vslContinued = vslContinuedIds(events).size;
  ...
  function renderActions(events) { const vslStarts = vslStartedIds(events).size; const items = [['Liberou o botão da VSL', countFor(events, 'vsl_continue_unlocked'), vslStarts], ['Avançou após a VSL', vslContinuedIds(events).size, vslStarts], ['Resgatou o voucher', countFor(events, 'voucher_redeemed'), countFor(events, 'step_viewed', (item) => Number(item.step_number) === 6)], ['Clicou no checkout', countFor(events, 'checkout_clicked'), countFor(events, 'step_viewed', (item) => Number(item.step_number) === 9)]]; elements.actionGrid.innerHTML = items.map(([label, count, base]) => '<article class="action-card"><strong>' + number(count) + '</strong><span>' + escapeHtml(label) + '</span><small>' + percent(count, base) + '% de conversão</small></article>').join(''); }
  function sessionStage(events) { let latest = 0; events.forEach((item) => { if (item.event_name === 'step_viewed') { const index = STAGES.findIndex((stage) => stage.step === Number(item.step_number)); if (index > latest) latest = index; } }); if (latest === 0 && vslStartedIds(events).size) latest = 1; return latest ? STAGES[latest].label : 'Entrada'; }
  function sessionVsl(events) { const progress = events.filter((item) => item.event_name === 'vsl_progress' || item.event_name === 'vsl_watch').map((item) => Number(item.metadata?.percent) || 0); if (progress.length) return Math.max(...progress) + '%'; if (events.some((item) => ['vsl_started', 'vsl_continue_unlocked', 'vsl_continue_clicked'].includes(item.event_name))) return 'Iniciou'; return events.some(isPostVslStep) ? 'Passou · dado antigo' : '—'; }
