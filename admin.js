(() => {
  ...
  const STAGES = [
    { step: 1, label: 'Promessa inicial' }, { step: 1.1, label: 'VSL' },
    { step: 2, label: 'Quebra de crença' }, { step: 3, label: 'Método na prática' },
    { step: 4, label: 'Crescimento do método' }, { step: 5, label: 'Benefícios + voucher' },
    { step: 6, label: 'Resgate do voucher' }, { step: 8, label: 'Módulos do treinamento' },
    { step: 9, label: 'Oferta' }
  ];
  ...
  function renderMetrics(sessions, events) {
    ...
    const vslStarted = countFor(events, 'step_viewed', (item) => Number(item.step_number) === 1.1);
    const vslContinued = countFor(events, 'vsl_continue_clicked');
    const offer = countFor(events, 'step_viewed', (item) => Number(item.step_number) === 9);
    ...
    elements.metricGrid.innerHTML = [
      ...
      metricCard('Avançaram após a VSL', number(vslContinued), percent(vslContinued, vslStarted) + '% de quem iniciou'), metricCard('Chegaram na oferta', number(offer), percent(offer, started) + '% das entradas'),
      ...
      metricCard('Abandono na VSL', percent(Math.max(0, vslStarted - vslContinued), vslStarted) + '%', number(Math.max(0, vslStarted - vslContinued)) + ' sessões'), metricCard('Sessões no período', number(sessions.length), 'Somente analytics V2')
    ].join('');
  }
  function renderFunnel(events) { const total = countFor(events, 'funnel_started'); let previous = total; elements.funnelList.innerHTML = STAGES.map(({ step, label }) => { const count = step === 1 ? total : countFor(events, 'step_viewed', (item) => Number(item.step_number) === step); const drop = previous ? Math.max(0, Math.round((previous - count) / previous * 100)) : 0; const width = Math.max(count ? 2 : 0, percent(count, total)); previous = count; return '<div class="funnel-row"><span class="step-number">' + step + '</span><span class="step-name">' + escapeHtml(label) + '</span><div class="funnel-track"><div class="funnel-fill" style="width:' + width + '%"></div></div><span class="funnel-count">' + number(count) + ' · ' + percent(count, total) + '%</span><span class="funnel-drop">-' + drop + '%</span></div>'; }).join(''); }
  function renderRetention(events) { const vslStarts = countFor(events, 'step_viewed', (item) => Number(item.step_number) === 1.1); elements.retentionGrid.innerHTML = VSL_POINTS.map((point) => { const count = countFor(events, 'vsl_progress', (item) => Number(item.metadata?.percent) === point); return '<article class="retention-card"><strong>' + point + '%</strong><span>' + number(count) + ' pessoas</span><small>' + percent(count, vslStarts) + '% retidas</small></article>'; }).join(''); }
  ...
  function renderSources(sessions, events) { const offerIds = idsFor(events, 'step_viewed', (item) => Number(item.step_number) === 9); const checkoutIds = idsFor(events, 'checkout_clicked'); const groups = [...groupBy(sessions, (item) => item.utm_source).entries()].sort((a, b) => b[1].length - a[1].length); elements.sourceTable.innerHTML = groups.slice(0, 12).map(([source, rows]) => { const offer = rows.filter((item) => offerIds.has(item.id)).length; const checkout = rows.filter((item) => checkoutIds.has(item.id)).length; return '<tr><td><strong>' + escapeHtml(source) + '</strong></td><td>' + number(rows.length) + '</td><td>' + number(offer) + ' (' + percent(offer, rows.length) + '%)</td><td>' + number(checkout) + ' (' + percent(checkout, rows.length) + '%)</td></tr>'; }).join('') || '<tr><td colspan="4" class="empty-state">Nenhuma origem encontrada.</td></tr>'; }
  function renderActions(events) { const vslStarts = countFor(events, 'step_viewed', (item) => Number(item.step_number) === 1.1); const items = [['Liberou o botão da VSL', countFor(events, 'vsl_continue_unlocked'), vslStarts], ['Clicou após a VSL', countFor(events, 'vsl_continue_clicked'), vslStarts], ['Resgatou o voucher', countFor(events, 'voucher_redeemed'), countFor(events, 'step_viewed', (item) => Number(item.step_number) === 6)], ['Clicou no checkout', countFor(events, 'checkout_clicked'), countFor(events, 'step_viewed', (item) => Number(item.step_number) === 9)]]; elements.actionGrid.innerHTML = items.map(([label, count, base]) => '<article class="action-card"><strong>' + number(count) + '</strong><span>' + escapeHtml(label) + '</span><small>' + percent(count, base) + '% de conversão</small></article>').join(''); }
  function sessionStage(events) { const viewed = events.filter((item) => item.event_name === 'step_viewed').sort((a, b) => Number(b.step_number) - Number(a.step_number)); const current = viewed[0]; return current ? STAGES.find((stage) => stage.step === Number(current.step_number))?.label || ('Tela ' + current.step_number) : 'Entrada'; }
  function sessionVsl(events) { const progress = events.filter((item) => item.event_name === 'vsl_progress').map((item) => Number(item.metadata?.percent) || 0); return progress.length ? Math.max(...progress) + '%' : (events.some((item) => Number(item.step_number) === 1.1) ? 'Iniciou' : '—'); }
  function renderSessions(sessions, events) { const bySession = groupBy(events, (item) => item.session_id); elements.sessionsTable.innerHTML = sessions.slice(0, 100).map((item) => { const rows = bySession.get(item.id) || []; const checkout = rows.some((event) => event.event_name === 'checkout_clicked'); const offer = rows.some((event) => event.event_name === 'step_viewed' && Number(event.step_number) === 9); const status = checkout ? '<span class="badge success">Checkout</span>' : offer ? '<span class="badge">Oferta</span>' : '<span class="badge muted">Em andamento / saiu</span>'; return '<tr><td>' + dateTime(item.started_at) + '</td><td>' + escapeHtml(item.utm_source || 'Direto') + '</td><td>' + escapeHtml(item.device_type || '—') + '</td><td>' + sessionVsl(rows) + '</td><td>' + escapeHtml(sessionStage(rows)) + '</td><td>' + status + '</td></tr>'; }).join('') || '<tr><td colspan="6" class="empty-state">Nenhuma sessão nova encontrada.</td></tr>'; }
